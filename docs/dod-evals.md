...
- `forge/runs/UNIT-03/20260722T151115Z-6fb52daf3ff1/run.json` at revision `6fb52daf3ff11630454c13a76adfd7875c749e8f` — canonical UNIT-03 evidence for S-10/S-11/S-12, real DOCX/TXT/bounded Google Docs conversion to validated EPUB+legacy MOBI, private-artifact and immutable `BookVersion`/`BookSubmitted` boundaries, failure/retry, 3/3 E2E, 30 visual screenshots, 7 accessibility receipts, 105 passed/2 skipped tests and zero high-severity npm audit findings. That receipt alone ends at Author submission; the downstream moderation/publication slice is separately passed by UNIT-04.
...
- `forge/runs/UNIT-05/20260727T215803Z-92e820ffc4e5/run.json` at revision `92e820ffc4e5c69752543457ad7fecf12e960c15` — canonical UNIT-05 evidence for real PostgreSQL migration `0006_commerce_checkout`, 28 Cart/order/payment/reconciliation/notification vectors, S-04/S-05/S-06, 3/3 Buyer E2E, 30 visual screenshots, 8 accessibility receipts, 129 passed/3 skipped tests and 0 audit vulnerabilities. The only finding is P3/advisory: credentialed live mono and Apple Pay/Google Pay presentation remain blocked for production-provider activation. The next executable unit is UNIT-06.
- `forge/runs/UNIT-06/20260729T002426Z-484a56dc4169/run.json` at revision `484a56dc41698dd8965e9b39b1cc79f25d91eca7` — canonical independent UNIT-06 evidence for reversible PostgreSQL migration `0007_library_reviews_refunds`, 7/7 entitlement/download/review/Refund proof vectors, 2/2 Buyer/Manager E2E, 12/12 S-07/S-08/S-09/S-20 responsive visual receipts, 129 passed/3 skipped tests and `findings: []`. Its verifier does not invoke or reuse `verify:unit05`; the next executable unit is UNIT-07.
...
|---|---|---|---|---|---|---|---|---|
| `build` | Production build збирається | guardrails Verification Rules; AD-6 | кожен юніт після bootstrap | свіжий `npm run build` лог | exit 0 | будь-яка помилка | після кожної зміни | automated; UNIT-00…UNIT-06 canonical builds passed, most recently revision `484a56dc41698dd8965e9b39b1cc79f25d91eca7` |
| `typecheck_lint` | Статична коректність | guardrails; AD-6 | кожен юніт після bootstrap | свіжі `npm run typecheck` і `npm run lint` логи | 0 помилок | помилки | після кожної зміни | automated; UNIT-06 typecheck/lint/repository and client-secret boundaries passed in `forge/runs/UNIT-06/20260729T002426Z-484a56dc4169/run.json` |
| `tests` | Автотести юніта й регресії | guardrails; AD-2/3/4/7/11 | кожен юніт після bootstrap | свіжий `npm test`; для affected journeys також `npm run test:e2e` | усі застосовні прогони зелені | будь-який failed/blocked застосовний тест | після кожного фіксу | automated; UNIT-06 records 25 test files passed/3 skipped, 129 tests passed/3 skipped, 2/2 Buyer/Manager E2E and 1/1 responsive visual suite |
...
| `paid_sale_only` | `PaidSale` лише з підтверджених оплат; повернення компенсуються окремою подією | FR-PAY-5, FR-REF-3 | commerce→library/rewards | тести paid/unpaid/cancelled/rollback/replay; UNIT-06 `RefundApproved` production and UNIT-07 ledger application | одна append-only versioned подія на paid Order; unpaid/cancelled produce none | sale без оплати, дубль або partial transition | після змін commerce | UNIT-05 producer side passed; UNIT-06 passed idempotent entitlement consumption plus exactly-one append-only compensation and `RefundApproved` without rewriting `PaidSale`; UNIT-07 must apply both events to its ledger |
...
| `access_separation` | Ролеві guard-и; sensitive data and internal moderation/payment signals remain within their boundaries; mutation Origin/CSRF fails closed | FR-MOD-3, FR-PAY-3, FR-REW-6, FR-FND-3, NFR-3; architecture Security | identity, moderation, commerce, library, rewards | negative role/API/UI tests; invalid Origin/session-CSRF/signature vectors; DTO/event/response inspection | unauthorized routes fail; invalid proofs mutate nothing; public/Author outputs omit internal signals; checkout/result/download are Buyer-owned; raw Cart/card data are absent | unauthorized read/write, state mutation after invalid proof, internal signal/ПД/token/card leak | після access/route/action/DTO changes | UNIT-06 passed guest/non-owner/signed-download separation, forged signature rejection, verified-Buyer review eligibility and Manager-only decisions; rewards/founder portions remain later |
| `webhook_idempotency` | Повторний/застарілий вебхук mono не дублює й не регресує продаж | AD-3 | commerce | exact-raw-body signature, duplicate/out-of-order/equal/missing timestamp and reconciliation tests | одна paid transition/event; invalid/misbound observation changes nothing; reconciliation converges | дубль, regression, signature/binding bypass or reconciliation gap | після змін інтеграції | passed by UNIT-05 real-PostgreSQL proof and 3/3 E2E at `forge/runs/UNIT-05/20260727T215803Z-92e820ffc4e5/run.json` |
...
| `journey_buyer_e2e` | Наскрізна покупка: кошик → вхід → mono → бібліотека → файли | journey покупця 1–7 | реліз, зміни commerce/library | e2e-прогін (тестовий режим mono) | шлях проходиться | розрив | перед релізом | UNIT-05 passes Cart→auth→mono→S-06→`PaidSale`; UNIT-06 passes the event handoff to Library, current approved EPUB/MOBI delivery, verified review and post-Refund revocation. The cross-unit contracts are closed; UNIT-10 still owns one deployment-context release replay |
...
| Gate | Purpose | Source References | Applies To | Required Evidence | Pass Condition | Fail Or Block Condition | Rerun Rule | Automation Status |
|---|---|---|---|---|---|---|---|---|
| `screen_states_coverage` | Стани кожного зачепленого екрана відповідають screen-map | screen-map Screen States | фронтенд-юніти | QA-чекліст IDs + state-bound E2E/captures | застосовні стани присутні | відсутній обовʼязковий стан | після змін екрана | UNIT-06 E2E passes authenticated Library, current-file/error authorization, review eligible/pending/published/hidden-after-Refund, Refund form/pending/approved and Manager request/approve; 12 representative revision-bound captures cover S-07/S-08/S-09/S-20 at 390/768/1280 |
| `accessibility_floor` | AA-підлога: контраст, фокус, клавіатура, підписи, цілі ≥44px | design-brief Accessibility Floor | фронтенд-юніти | виміряний contrast/targets/labels + keyboard/focus/reflow receipts | застосовні вимірювання проходять; state/dialog/list-detail зміни доступні без миші | blocking threshold/focus/label/reflow failure | після змін UI | UNIT-05 passed 8 named keyboard/status/reflow checks; minimum touch target 44×44, control contrast 4.216, text contrast 14.025 and mobile input font 16px; no complete WCAG/release-browser claim |
| `responsive_viewports` | 390/430/768/1280/1440 без горизонтального скролу й втрати пріоритетів | design-brief Responsive; wireframes Responsive Notes | фронтенд-юніти | скріншоти вʼюпортів + mobile interaction receipts | структура за wireframes and owning-screen priorities | зламаний вʼюпорт або недоступний control | після змін layout | UNIT-05 passed 30 captures across five widths, 200% reflow and maximum overflow 0 for S-04/S-05/S-06 |
...
- Automation Status: UNIT-01/UNIT-02-C1/UNIT-03 retain their scoped passed receipts. UNIT-04 extension mode passed S-13/S-18/S-02 unavailable. UNIT-05 extension mode passed S-04/S-05/S-06 with 30 screenshots at 390/430/768/1280/1440 and 8 named accessibility checks; no S-01 pixel-lock or provider-owned checkout-UI claim is made. Other screens remain governed by their owning units.
...
| `live_mono_provider_smoke` | Credentialed merchant checkout, registered webhook and named Apple Pay/Google Pay presentation work in sandbox/pre-production | FR-PAY-3/5; AD-3; BD-02 | активація production mono / public release | credentialed provider run against the exact production adapter revision | checkout, webhook/status convergence and applicable wallet methods complete without contract/config drift | credentials/registration unavailable or any applicable method fails | before first activation and after mono config/adapter changes | blocked for provider activation; UNIT-05 passed signed local protocol simulation but explicitly did not claim live credentials or wallet presentation |
...
- UNIT-05 commerce/notification claim additionally requires one clean-revision bundle combining reversible real PostgreSQL migration `0006`, all 28 Cart/order/payment/reconciliation/notification vectors, negative ownership/signature/binding/privacy checks, S-04/S-05/S-06 success/failure/missed-webhook E2E, sanitized E-01 evidence, exactly 30 visual receipts and all 8 named accessibility receipts. `forge/runs/UNIT-05/20260727T215803Z-92e820ffc4e5/run.json` satisfies this contract; partial future runs cannot be combined into a pass without the canonical verifier.
...
- UNIT-05 evidence remains limited to Cart/payment/`PaidSale`/E-01. UNIT-06 independently closes `LibraryEntitlement`, purchased-file delivery, verified-Buyer reviews and Refund/`RefundApproved`; it does not claim rewards/ledger application, production S3 deployment, production transactional-email configuration, credentialed mono checkout, Apple Pay/Google Pay presentation, production tariffs, or complete WCAG/cross-browser/device release coverage.
...
- No additional DoD decision blocks development planning. Conversion, UNIT-04 moderation/publication, UNIT-05 Cart/payment/`PaidSale`/E-01 and UNIT-06 Library/review/Refund proofs are closed; upstream release gates OQ-1/OQ-2 and UNIT-07…UNIT-10 stay explicit.
