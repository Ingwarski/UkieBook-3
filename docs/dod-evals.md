...
|---|---|---|---|---|---|---|---|---|
| `build` | Production build збирається | guardrails Verification Rules; AD-6 | кожен юніт після bootstrap | свіжий `npm run build` лог | exit 0 | будь-яка помилка | після кожної зміни | automated; UNIT-00…UNIT-05 canonical builds passed, most recently revision `92e820ffc4e5c69752543457ad7fecf12e960c15` |
| `typecheck_lint` | Статична коректність | guardrails; AD-6 | кожен юніт після bootstrap | свіжі `npm run typecheck` і `npm run lint` логи | 0 помилок | помилки | після кожної зміни | automated; UNIT-05 typecheck/lint/repository and client-secret boundaries passed in `forge/runs/UNIT-05/20260727T215803Z-92e820ffc4e5/run.json` |
| `tests` | Автотести юніта й регресії | guardrails; AD-2/3/4/7/11 | кожен юніт після bootstrap | свіжий `npm test`; для affected journeys також `npm run test:e2e` | усі застосовні прогони зелені | будь-який failed/blocked застосовний тест | після кожного фіксу | automated; UNIT-05 records 25 test files passed/3 skipped, 129 tests passed/3 skipped, 3/3 Buyer E2E and 1/1 visual suite; `npm audit` reports 0 vulnerabilities |
...
| `paid_sale_only` | `PaidSale` лише з підтверджених оплат; майбутні повернення компенсуються окремою подією | FR-PAY-5, FR-REF-3 | commerce→library/rewards | тести paid/unpaid/cancelled/rollback/replay; UNIT-06 `RefundApproved` production and UNIT-07 ledger application | одна append-only versioned подія на paid Order; unpaid/cancelled produce none | sale без оплати, дубль або partial transition | після змін commerce | UNIT-05 producer side passed: atomic/append-only/rollback-safe `PaidSale`, one sale per paid Order, none for unpaid/cancelled; UNIT-06 still must produce idempotent `RefundApproved`, and UNIT-07 must apply its financial compensation |
...
| `access_separation` | Ролеві guard-и; sensitive data and internal moderation/payment signals remain within their boundaries; mutation Origin/CSRF fails closed | FR-MOD-3, FR-PAY-3, FR-REW-6, FR-FND-3, NFR-3; architecture Security | identity, moderation, commerce, rewards | negative role/API/UI tests; invalid Origin/session-CSRF/signature vectors; DTO/event/response inspection | unauthorized routes fail; invalid proofs mutate nothing; public/Author outputs omit internal signals; checkout/result are Buyer-owned; raw Cart/card data are absent | unauthorized read/write, state mutation after invalid proof, internal signal/ПД/token/card leak | після access/route/action/DTO changes | UNIT-01 identity and UNIT-04 moderation portions passed their applicable Origin/CSRF vectors; UNIT-05 passed Buyer ownership, forged-signature rejection and raw-token/card non-persistence only, without claiming an additional Origin/CSRF receipt; rewards/founder portions remain later |
...
| `journey_buyer_e2e` | Наскрізна покупка: кошик → вхід → mono → бібліотека → файли | journey покупця 1–7 | реліз, зміни commerce/library | e2e-прогін (тестовий режим mono) | шлях проходиться | розрив | перед релізом | UNIT-05 passed Cart→auth merge→mono→S-06→`PaidSale`/E-01 plus failure and missed-webhook recovery; Library entitlement/files remain UNIT-06, so the combined release journey is not yet passed |
...
| Gate | Purpose | Source References | Applies To | Required Evidence | Pass Condition | Fail Or Block Condition | Rerun Rule | Automation Status |
|---|---|---|---|---|---|---|---|---|
| `screen_states_coverage` | Стани кожного зачепленого екрана відповідають screen-map | screen-map Screen States | фронтенд-юніти | QA-чекліст IDs + state-bound E2E/captures | всі стани присутні | відсутній обовʼязковий стан | після змін екрана | UNIT-04 retains 14 S-13/S-18/S-02 states/50 receipts; UNIT-05 passed S-04 empty/auth-required/error/populated, S-05 redirecting and S-06 pending/failure/success with 30 receipts |
...
- UNIT-05 evidence applies only to Cart/payment/`PaidSale`/E-01 through the UNIT-06 handoff. It does not claim `LibraryEntitlement`, purchased-file delivery, reviews, Refund/`RefundApproved`, rewards/ledger, production transactional-email configuration, credentialed mono checkout, Apple Pay/Google Pay presentation, production tariffs, or complete WCAG/cross-browser/device release coverage.
...
- No additional DoD decision blocks development planning. Conversion, UNIT-04 moderation/publication and UNIT-05 Cart/payment/`PaidSale`/E-01 proofs are closed; upstream release gates OQ-1/OQ-2 and UNIT-06…UNIT-10 stay explicit.
