...
- `forge/runs/UNIT-03/20260722T151115Z-6fb52daf3ff1/run.json` at revision `6fb52daf3ff11630454c13a76adfd7875c749e8f` — canonical UNIT-03 evidence for S-10/S-11/S-12, real DOCX/TXT/bounded Google Docs conversion to validated EPUB+legacy MOBI, private-artifact and immutable `BookVersion`/`BookSubmitted` boundaries, failure/retry, 3/3 E2E, 30 visual screenshots, 7 accessibility receipts, 105 passed/2 skipped tests and zero high-severity npm audit findings. The receipt ends at Author submission; UNIT-04 Manual Review, publication activation and public catalog projection are not passed. Next executable unit is UNIT-04.
- Migration `0005_moderation_publication`, `modules/moderation/`, UNIT-04 S-13/S-18/S-02 routes, `scripts/verify-unit04-postgres.ts`, Playwright configs/specs and `scripts/run-unit04-verification.mjs` — implemented verification surface for UNIT-04. The canonical clean-revision `npm run verify:unit04` result is pending; this artifact does not assign a pass status or invent a receipt path before that run.
...
|---|---|---|---|---|---|---|---|---|
| `build` | Production build збирається | guardrails Verification Rules; AD-6 | кожен юніт після bootstrap | свіжий `npm run build` лог | exit 0 | будь-яка помилка | після кожної зміни | automated; UNIT-00…UNIT-02 historical runs and UNIT-02-C1 passed; UNIT-03 build passed at revision `6fb52daf3ff11630454c13a76adfd7875c749e8f` in its canonical run |
| `typecheck_lint` | Статична коректність | guardrails; AD-6 | кожен юніт після bootstrap | свіжі `npm run typecheck` і `npm run lint` логи | 0 помилок | помилки | після кожної зміни | automated; UNIT-00…UNIT-02 historical runs and UNIT-02-C1 passed; UNIT-03 typecheck/lint and repository boundary review passed in `forge/runs/UNIT-03/20260722T151115Z-6fb52daf3ff1/run.json` |
| `tests` | Автотести юніта й регресії | guardrails; AD-2/3/4/7/11 | кожен юніт після bootstrap | свіжий `npm test`; для affected journeys також `npm run test:e2e` | усі застосовні прогони зелені | будь-який failed/blocked застосовний тест | після кожного фіксу | automated; UNIT-03 records 19 test files passed/2 skipped, 105 tests passed/2 skipped, 3/3 UNIT-03 E2E and 1/1 visual suite; `npm audit --audit-level=high` reports 0 vulnerabilities |
...
| Gate | Purpose | Source References | Applies To | Required Evidence | Pass Condition | Fail Or Block Condition | Rerun Rule | Automation Status |
|---|---|---|---|---|---|---|---|---|
| `journey_author_e2e` | Наскрізна публікація: завантаження → Попередній перегляд видання → окремі права/ліцензія → модерація → Каталог | journey Автора 3–9 | реліз, зміни publishing/moderation | revision-bound combined evidence: UNIT-03 submit receipt + UNIT-04 real-state/E2E lifecycle proof | safe and risky paths reach authoritative S-13 and public states; rejection exposes only ReasonCategory; Автор не потрапляє в Manager UI | розрив шляху/обхід підтверджень або рольової межі | перед релізом; після змін шляху | UNIT-03 segment through `BookSubmitted` passed; UNIT-04 extension harness is implemented and its canonical result is pending; release-candidate rerun remains required |
...
| Gate | Purpose | Source References | Applies To | Required Evidence | Pass Condition | Fail Or Block Condition | Rerun Rule | Automation Status |
|---|---|---|---|---|---|---|---|---|
| `screen_states_coverage` | Стани кожного зачепленого екрана відповідають screen-map | screen-map Screen States | фронтенд-юніти | QA-чекліст IDs + state-bound E2E/captures | всі стани присутні | відсутній обовʼязковий стан | після змін екрана | prior owning routes and UNIT-03 scopes passed. UNIT-04 harness covers S-13 submitted/manual/rejected/published/removed; S-18 mixed/type-selected/provider-error/category-error/removal/empty; S-02 unavailable. Canonical UNIT-04 result pending |
| `accessibility_floor` | AA-підлога: контраст, фокус, клавіатура, підписи, цілі ≥44px | design-brief Accessibility Floor | фронтенд-юніти | виміряний contrast/targets/labels + keyboard/focus/reflow receipts | застосовні вимірювання проходять; state/dialog/list-detail зміни доступні без миші | blocking threshold/focus/label/reflow failure | після змін UI | prior scopes and UNIT-03 passed. UNIT-04 expects 8 named receipts: S-13 keyboard + 200% reflow; S-18 queue keyboard, reason focus, removal-dialog trap/return, mobile list/detail/back, 200% reflow; S-02 unavailable 200% reflow. Canonical UNIT-04 result pending; no complete WCAG claim |
| `responsive_viewports` | 390/430/768/1280/1440 без горизонтального скролу й втрати пріоритетів | design-brief Responsive; wireframes Responsive Notes | фронтенд-юніти | скріншоти вʼюпортів + mobile interaction receipts | структура за wireframes; S-18 mobile is explicit list→detail→back | зламаний вʼюпорт, недоступний control або відсутній mobile return | після змін layout | prior scopes and UNIT-03 passed. UNIT-04 expects 50 screenshots across five widths plus mobile S-18 list/detail/back and 200% reflow receipts; canonical result pending |
...
- Automation Status: UNIT-01/UNIT-02-C1/UNIT-03 retain their scoped passed receipts. UNIT-04 extension-mode automation is implemented for S-13, S-18 and S-02 unavailable with 50 expected screenshots at 390/430/768/1280/1440 and 8 named accessibility checks; no S-01 pixel-lock claim is made. Its canonical run/status is pending. Other screens and the UNIT-05-owned Cart destination/behavior remain governed by their owning units.
...
- UNIT-04 source/tests define a real-PostgreSQL, E2E and visual evidence contract, but an implemented harness or non-canonical local check is not completion evidence. Until a clean-revision `npm run verify:unit04` seals `run.json`, every UNIT-04 gate above remains pending and no receipt path is cited.
...
- No additional DoD decision blocks development planning. The conversion-engine proof is closed by UNIT-03; UNIT-04 canonical verification, upstream release gates OQ-1/OQ-2 and UNIT-05…UNIT-10 stay explicit.
