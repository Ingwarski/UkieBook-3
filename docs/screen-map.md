...
| S-11 | Майстер публікації: Рукопис → опис/Ілюстрації → Обкладинка → Жанр/ціна → Декларація прав + ліцензійна умова; крок 5 відкриває S-12 | `/author/publish` | FR-PUB-1..9, FR-LIC-2/3 |
| S-12 | Крок 5 майстра: завершений Попередній перегляд Адаптивного видання і Сторінки книжки + обовʼязковий вибір Безкоштовного фрагмента з реальних розділів цього результату | `/author/publish/preview` | FR-PUB-4/6, A-3 |
| S-13 | Керування книжкою Автора: життєвий цикл публікації, Категорія причини відхилення, посилання на публічну/архівну S-02; повний продуктовий екран також охоплює керування Знижкою й запуск Оновлення книжки | `/author/books/{id}` | FR-MOD-3, FR-CAT-4, FR-UPD-1, FR-LIC-4 |
...
| ID | Екран | Маршрут | Джерело |
|---|---|---|---|
| S-18 | Черга Ручної перевірки: ризикові Книжки, Оновлення книжки й відгуки; safe handoff при недоступному провайдері ШІ-модерації; рішення за типом; керування доступністю ризикової опублікованої Книжки | `/admin/moderation` | FR-MOD-1/2/4/5, FR-LIC-4 |
...
Межа поетапного надання можливостей не звужує канонічний скоуп екрана: у UNIT-04 S-13 має lifecycle/public-link slice (`На модерації` → `На ручній перевірці` / `Відхилено` / `Опубліковано` / `Прибрано з Каталогу`) без форм Знижки та Оновлення книжки; ці комерційні/оновлювальні зони належать пізнішому slice UNIT-08. Статус завершеності юнітів і докази належать development-plan/DoD, а не цьому screen-map.
...
flowchart LR
  ...
  S11 -->|крок 4: Жанр і ціна; підготувати результат| S12[Попередній перегляд видання + вибір фрагмента]
  S12 -->|фрагмент збережено; крок 6: права й ліцензія| S11
  S12 -->|повернення до редагування| S11
  S11 -->|подано| S10
  S11 -.->|BookSubmitted| AI[ШІ-модерація]
  AI -->|без ризику: автопублікація| S02
  AI -.->|ризик або провайдер недоступний| S18[Черга Ручної перевірки]
  ...
  S18 -.->|FR-LIC-4: прибрано| U02[S-02: Книжка недоступна]
...
| Автор 3–5 (матеріали, обкладинка, жанр/ціна) | S-11 |
| Автор 5–6 (Попередній перегляд видання та вибір реального розділу як Безкоштовного фрагмента) | S-12 |
| Автор 7 (Декларація прав, ліцензійна умова, подання) | S-11 |
| Автор 8 (модерація) | S-13 (лише зовнішній lifecycle/status і Категорія причини), S-18 (внутрішні сигнали й рішення Менеджера) |
...
| S-02 | завантаження; книжка недоступна після прибирання за FR-LIC-4: маршрут лишається архівно доступним, нові покупки й Безкоштовний фрагмент недоступні, уже придбані EPUB/MOBI лишаються в Бібліотеках; фрагмент відкрито для доступної Книжки; знижка активна/неактивна; довгі відгуки (пагінація) |
...
| S-10 | немає книжок (перший вхід); список зі статусами: чернетка / на модерації / ручна перевірка / відхилено (Категорія причини) / опубліковано / прибрано з Каталогу |
| S-11 | кроки майстра; завантаження файлу; помилка формату (не DOCX/TXT/GDocs); зламаний файл; збереження Чернетки; крок 4 «Жанр і ціна»; Декларація прав прийнята/не прийнята; п'ятирічна ліцензійна умова прийнята/не прийнята |
| S-12 | формується Попередній перегляд видання; помилка конвертації; завершений результат без обраного Безкоштовного фрагмента (перехід до кроку 6 недоступний); реальний розділ обрано й збережено; вибір застарів після зміни вмісту чи нової конвертації та потребує повтору; повернення до редагування |
| S-13 | lifecycle slice: `На модерації` (`submitted`), `На ручній перевірці` (`manual_review_pending`), `Відхилено` (`rejected`, з однією Категорією причини із закритого нейтрального списку), `Опубліковано` (`published`, із посиланням на S-02), `Прибрано з Каталогу` (`unavailable`, із посиланням на архівну S-02 та поясненням, що нові покупки закриті, а придбані файли збережені); внутрішні сигнали ШІ не показуються Автору. Пізніший повний стан екрана: Знижка активна/запланована/завершена; Оновлення книжки в черзі/очікує накопичення 250 грн (FR-UPD-2) |
...
| S-18 | порожня черга; список/деталь Ризикових випадків із фільтром `Книжка / Оновлення книжки / відгук`; окремий safe-fail стан `ШІ-скринінг недоступний`, який завжди потребує рішення Менеджера; для Книжки — `Схвалити й опублікувати` або `Відхилити книжку` з обовʼязковою Категорією причини із закритого списку; для Оновлення книжки — `Схвалити оновлення` або `Відхилити оновлення` з такою Категорією причини; для відгуку — `Опублікувати відгук` або `Не публікувати` без Категорії причини; для ризикової опублікованої Книжки — `Залишити в Каталозі` або окремо підтверджене `Прибрати з Каталогу` лише з підставою FR-LIC-4, без Категорії причини; успіх/помилка/конфлікт рішення; mobile list → detail → повернення до того самого відфільтрованого списку |
...
- Крок 4 S-11 зберігає Жанр і ціну та запускає підготовку Попереднього перегляду видання; він не показує вигаданого або наперед обраного розділу. Після завершення конвертації крок 5 відкривається в S-12.
- S-12 дозволяє обрати Безкоштовний фрагмент лише з реальних розділів завершеного Попереднього перегляду видання. Перехід до кроку 6 заблокований, доки вибір не збережено; зміна Рукопису, опису, Ілюстрацій, Обкладинки, Жанру чи ціни або нова конвертація скасовує попередній вибір і вимагає повторного вибору в S-12.
- S-12 → повернення на будь-який попередній крок S-11 без втрати даних (journey, стадія 6); повернення після збереженого вибору може зробити його нечинним за правилом вище.
- S-11 → подання можливе лише після чинного вибору Безкоштовного фрагмента в S-12 та двох окремих підтверджень: Декларації прав (FR-PUB-8) і п'ятирічної ліцензійної умови (FR-LIC-2/3).
- Подання S-11 створює `BookSubmitted`: безризиковий результат ШІ-модерації активує публікацію й переводить S-13 у `Опубліковано`; ризиковий результат або недоступність провайдера переводить S-13 у `На ручній перевірці` та створює S-18. Автор бачить лише зовнішній статус/Категорію причини, не сигнал чи внутрішнє правило.
- У S-18 Категорія причини є обовʼязковою тільки для відхилення Книжки або Оновлення книжки й обирається із закритого нейтрального списку. Рішення `Не публікувати` для відгуку не запитує Категорію причини. Прибирання вже опублікованої Книжки використовує тільки одну підставу FR-LIC-4 та явне підтвердження; Категорія причини до цієї дії не домішується.
- На mobile S-18 відкриття випадку переходить зі списку до detail-поверхні; `До черги` повертає до того самого фільтра типу. Desktop зберігає master-detail без зміни контрактів рішень.
...
- Застарілий вибір Безкоштовного фрагмента: зміна вмісту або повторна конвертація очищує вибір → S-12 показує завершений результат і вимагає повторно обрати реальний розділ; прямий перехід до кроку 6 не обходить цю вимогу.
...
- Недоступний провайдер ШІ-модерації: `BookSubmitted` не автосхвалюється й не відхиляється → S-13 «На ручній перевірці» → S-18 із менеджерським safe-fail повідомленням; технічний збій не стає Авторською Категорією причини.
