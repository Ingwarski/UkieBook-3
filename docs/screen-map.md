...
| S-15 | Винагорода: продажі, нарахування 65,8%, формула, статуси виплат, статистика за книжкою | `/author/rewards` | FR-REW-5/6 |
...
| S-19 | Таблиця виплат: рядок автор×місяць, підтвердження кожної виплати | `/admin/payouts` | FR-PYT-1..4 |
...
- Ролі розмежовано: маршрути `/author/*` — лише автор; `/admin/*` — лише менеджер; `/library` — автентифікований покупець.
...
| S-19 | таблиця за місяць; рядок: сума продажів, % податків, % Автора, сума до Виплати; статуси: очікує / виплачено / перенесено; порожній місяць |
...
Права доступу: гість на `/author/*`, `/admin/*`, `/library` → редирект на S-03; автор на `/admin/*` → відмова в доступі; покупець без покупки книжки → S-08 приховано (FR-REV-1).
