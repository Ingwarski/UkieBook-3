...
- `docs/architecture.md` — AD-2/3/4/6/7/8/9/10/11/12 and security boundaries.
...
- Migration `0006_commerce_checkout`, `modules/commerce/`, `modules/notifications/`, S-04/S-05/S-06 routes, mono/payment API, `scripts/verify-unit05-postgres.ts`, `tests/e2e-unit05/`, `tests/visual-unit05/` and `scripts/run-unit05-verification.mjs` — canonical UNIT-05 verification surface passed at revision `92e820ffc4e5c69752543457ad7fecf12e960c15`; receipt `forge/runs/UNIT-05/20260727T215803Z-92e820ffc4e5/run.json`.

Product-route, journey and release checks below remain `blocked` until their owning units produce fresh implementation evidence. UNIT-00 platform foundation, UNIT-01 identity/profile, UNIT-02 behavior/persistence, UNIT-02-C1 V3 visual correction, UNIT-03 publishing/conversion, UNIT-04 moderation/publication and UNIT-05 commerce/notification execution recorded below are scoped passed exceptions. Baseline approval and test source code alone do not pass runtime checks.
...
- **Historical Next Unit:** UNIT-03, UNIT-04, UNIT-05 and UNIT-06 are now completed; the current next executable unit is UNIT-07.
...
### UNIT-05 Commerce And Purchase-Notification Execution

- **Verification Status:** `passed` at revision `92e820ffc4e5c69752543457ad7fecf12e960c15`; canonical evidence `forge/runs/UNIT-05/20260727T215803Z-92e820ffc4e5/run.json`.
- **Passed Scope:** migration `0006`; persistent guest and Buyer Cart; deduplicating OAuth-return merge with raw-token deletion; immutable Order/OrderItem financial snapshots; one mono invoice per active checkout; HTTPS production adapter and bounded provider responses; exact-raw-body ECDSA webhook with key rotation; invoice/reference/amount/currency binding; idempotent/out-of-order-safe observations; durable reconciliation through missing webhook, missing/equal timestamp and outage; atomic append-only `PaidSale`; isolated/idempotent E-01; S-04/S-05/S-06 success/failure/pending. Library entitlement/files/reviews/Refund remain UNIT-06.
- **Scoped Global IDs:** ACC-09 multi-book/redirect/failure/idempotency subset; UJ-02 through the UNIT-06 Library handoff; UJ-03 payment-failure preservation; ST-02 S-04/S-06 portion; INT-01 commerce portion; RES-03 S-04 portion; applicable A11Y/VIS checks. Credentialed card/wallet presentation, Library/files and the combined release Buyer journey are not passed by this unit.

### UNIT-06 Execution

- **Result:** `passed` at implementation revision `484a56dc41698dd8965e9b39b1cc79f25d91eca7`.
- **Canonical Evidence:** `forge/runs/UNIT-06/20260729T002426Z-484a56dc4169/run.json`; independent verifier `npm run verify:unit06` does not invoke `verify:unit05`.
- **Passed Scope:** reversible migration `0007_library_reviews_refunds`; idempotent `PaidSale`→`LibraryEntitlement`; Buyer-only signed EPUB/MOBI; active-approved-version resolution; verified-Buyer review→manual moderation→public projection; Refund request/Manager approval; exactly one append-only compensation and `RefundApproved`; post-Refund download/review revocation; S-07/S-08/S-09/S-20 responsive Aurora evidence.
- **Evidence Counts:** PostgreSQL 7/7 vectors; Vitest 129 passed/3 skipped; Buyer/Manager E2E 2/2; visual 12/12 across 390/768/1280; zero console/page errors, horizontal overflow, secret leaks and trace/HAR artifacts; `findings: []`.
- **Scoped Global IDs:** ACC-10/11; UJ-02 Library handoff and UJ-04 Refund; ST-03/09; INT-01 Library consumer and Refund producer portions; RES-03/05; applicable A11Y/VIS checks. Rewards-ledger application, production S3 and release-wide provider/device coverage remain later gates.
- **Functional Evidence:** dedicated PostgreSQL `ukiebook_unit05` passed rollback/reapply plus 28 Cart/order/payment/reconciliation/notification vectors; 129 tests passed/3 skipped; E2E 3/3 covered two-book success, failed-payment Cart preservation and missed-webhook recovery; all 12 canonical commands exited 0; repository/client/evidence secret hygiene passed and `npm audit` found 0 vulnerabilities.
- **Visual Evidence:** 30/30 revision-bound screenshots cover S-04 empty/auth-required/error/populated, S-05 redirecting and S-06 pending/failure/success across 390/430/768/1280/1440; visual digest `808a42ae719425919c29e8e1a8b8df7c366c7bcc864922a77dba789ec589b342`; zero console/page errors and maximum horizontal overflow 0. Provider-owned payment entry is intentionally not a pixel target.
- **Accessibility Evidence (8/8):** `s04-keyboard-order-focus-activation`, `s04-remove-item-focus-status`, `s04-reflow-200`, `s05-transition-status-announcement`, `s06-status-announcement`, `s06-success-keyboard-activation`, `s06-failure-keyboard-activation`, `s06-reflow-200`; measured targets are at least 44×44 CSS px, minimum control contrast 4.216, text contrast 14.025 and mobile input text 16px.
- **External Chrome Evidence:** exact Buyer route exercised with two guest books, Google simulator auth/Cart merge, one 398 UAH mono simulator invoice and successful S-06 result; Cart became empty, console errors were 0 and horizontal overflow was 0. The browser controller could not submit the local simulator HTML form, so only that simulator-control transition was completed through its test-only control endpoint before Chrome verified the result; canonical Playwright E2E separately passed the actual UI/provider transitions.
- **Activation Limit:** one P3/advisory finding: credentialed live mono checkout, registered webhook and Apple Pay/Google Pay presentation are deferred to sandbox/pre-production activation. Production transactional email also remains unavailable until a real provider is configured. Neither is claimed by the local signed-protocol evidence.
...
## UNIT-05 Scoped Checks

These checks instantiate the global IDs only for the bounded UNIT-05 slice. Every `passed` result below is bound to `forge/runs/UNIT-05/20260727T215803Z-92e820ffc4e5/run.json`; it does not expand into Library/reviews/Refund/rewards or production-provider activation.

| Check ID | Applicability | Check | Severity / Release Effect | Source | Required Evidence | Rationale | Result |
|---|---|---|---|---|---|---|---|
| U05-PG-01 | schema/runtime | PostgreSQL rolls migration `0006_commerce_checkout` down/up and proves all 28 Cart/order/payment/reconciliation/notification invariants | P1 / blocking | AD-3/7; migration `0006` | clean-revision dedicated-loopback PostgreSQL receipt | code inspection cannot prove PostgreSQL constraints, triggers, locks and transactions | passed |
| U05-CART-01 | Guest/Buyer S-04 | Cart persists by hashed opaque guest token, deduplicates, merges at OAuth return, clears the raw cookie and rejects unavailable Books | P1 / blocking | FR-PAY-1/2; ST-02 | real-PG vectors + two-book browser E2E | preserves intent without persisting a bearer token | passed |
| U05-ORDER-01 | checkout | Order/OrderItem title/Author/Cover/price snapshots and total are immutable/serializable; repeat checkout reuses the active session | P1 / blocking | FR-PAY-2/5; INT-01 | database constraints + concurrent/repeat vectors | prevents price drift and duplicate invoices | passed |
| U05-MONO-01 | provider adapter/webhook | Create/status/public-key responses are bounded and invoice-bound; webhook uses exact raw-body ECDSA, rotation and stale/out-of-order/equal/missing timestamp guards | P1 / blocking | FR-PAY-3/5; AD-3 | adapter tests + signed simulator + real-PG vectors | prevents spoofed/misrouted/regressive payment transitions | passed |
| U05-RECON-01 | missed webhook/outage | Authoritative status reconciliation converges once and always leaves a durable successor/overdue issue until terminal recovery | P1 / blocking | FR-PAY-5; AD-3/7 | missed-webhook E2E + horizon/outage vectors | provider delivery is not a correctness dependency | passed |
| U05-SALE-01 | paid/unpaid/cancelled | Successful terminal transition atomically appends one paid Order, one `PaidSale`, notification request and outbox; injected failure rolls all back; unpaid/cancelled produce no sale | P1 / blocking | FR-PAY-4/5; INT-01 | atomicity/rollback/idempotency vectors + E2E | downstream UNIT-06/07 must consume one trustworthy event | passed |
| U05-EMAIL-01 | E-01 | Captured email contains every purchased title and `/library`; delivery is isolated/idempotent and cannot roll back `PaidSale` | P1 / blocking | FR-PAY-4; E-01 | sanitized capture + notification isolation/failure vectors | communication failure cannot destroy paid value | passed |
| U05-SEC-01 | checkout/result/webhook | Buyer ownership, forged signature, provider binding and bounded-body/provider-response checks fail closed; card data and raw Cart token are absent from persistence/evidence | P1 / blocking | FR-PAY-3; NFR-3 | negative tests + database/evidence scans | payment and privacy boundary | passed |
| U05-A11Y-01 | S-04/S-05/S-06 | All 8 named keyboard/status/reflow checks pass; targets ≥44×44, mobile inputs ≥16px, measured contrast meets thresholds and no overflow/errors occur | P2 / blocking | design-brief Accessibility Floor; A11Y-01..08 | 8 revision-bound receipts + per-capture measurements | screenshots alone do not prove interaction/state announcements | passed |

| Check ID | Baseline ID / Target Hash | Route / State / Viewport | Severity / Release Effect | Permitted Variance | Expected Evidence | Rationale | Result |
|---|---|---|---|---|---|---|---|
| U05-VIS-30 | `AVB-UKIEBOOK-AURORA-7B-V3` / `e50c9f82c241195d7f5d8876d9dcdcd7fd45b71cdaf6d2eedfe2e327a7182724` | S-04 four states; S-05 redirecting; S-06 pending/failure/success; core widths 390/768/1280 plus coverage at 430/1440; exactly 30 captures | P2 / blocking | Aurora Public extension; provider-owned payment entry excluded; no S-01 pixel target | hashed revision-bound matrix; 30 distinct files; zero console/page errors; overflow/touch/a11y measurements | proves the complete owned commerce UI state matrix without inventing provider pixel targets | passed |
...
| ST-03 | S-07/S-08/S-09 | Library ownership/latest version/download error; review eligibility/pending/published; Refund form/pending/approved | P1 / blocking | screen-map | UNIT-06 PostgreSQL + 2/2 e2e + 12 responsive captures | passed in UNIT-06 canonical scope |
...
| ST-09 | S-20 | request/approve; decision updates Library and emits ledger input | P1 / blocking | screen-map | UNIT-06 PostgreSQL + Manager e2e + responsive captures | passed producer/UI scope; UNIT-07 applies ledger |
...
| VIS-AURORA-PUBLIC | `AVB-UKIEBOOK-AURORA-7B-V3` / `e50c9f82c241195d7f5d8876d9dcdcd7fd45b71cdaf6d2eedfe2e327a7182724` | S-02…S-09, applicable states, 390/768/1280 | P2 / blocking | design-spine extension, real content/state structure | token audit + representative state captures | no pixel target; system contract applies | S-02 scoped passed by UNIT-02-C1/UNIT-04; S-04/S-05/S-06 by UNIT-05; S-07/S-08/S-09 by UNIT-06 with 9 revision-bound public receipts; S-03 remains UNIT-01 history |
...
| VIS-TOKENS | `AVB-UKIEBOOK-AURORA-7B-V3` / `e50c9f82c241195d7f5d8876d9dcdcd7fd45b71cdaf6d2eedfe2e327a7182724` | all visible routes, default and semantic states, 390/768/1280/1440 | P2 / blocking for hierarchy or brand drift | only source-backed contrast corrections and documented state colors | production token export/computed-style matrix + representative captures against V3 values | dedicated proof for Aurora palette, gradients, type, radii, spacing and shadows | S-01/S-02 scoped by UNIT-02-C1; UNIT-03 S-10/S-11/S-12, UNIT-04 S-02/S-13/S-18 and UNIT-05 S-04/S-05/S-06 passed extension captures and measured contrast without changing shared token definitions or claiming a separate token computed-style receipt; rerun on any shared-token change |
| VIS-GLASS | `AVB-UKIEBOOK-AURORA-7B-V3` / `e50c9f82c241195d7f5d8876d9dcdcd7fd45b71cdaf6d2eedfe2e327a7182724` | glass cards/panels on applicable public, Author and Manager routes, default/hover/focus, 390/768/1280 | P2 / blocking for hierarchy or system drift | density may change only where the extension contract permits it | computed background/border/blur/radius/shadow + component captures matched to V3 glass evidence | glassmorphism is a named cross-screen invariant | S-01/S-02 scoped by UNIT-02-C1; UNIT-03 reviewed S-10/S-11/S-12, UNIT-04 reviewed S-02-unavailable/S-13/S-18 and UNIT-05 reviewed S-04/S-05/S-06 glass application in their extension matrices, without claiming a separate shared-primitive computed-style artifact; rerun when the shared glass primitive changes |
| VIS-COVER | `AVB-UKIEBOOK-AURORA-7B-V3` / `e50c9f82c241195d7f5d8876d9dcdcd7fd45b71cdaf6d2eedfe2e327a7182724` | every Book Cover, default/hover/loading, 390/768/1280/1440 | P2 / blocking | real Covers remain 2:3, every corner radius is exactly `0`, artwork title is baked into image and no live title overlay is allowed | computed radius/aspect assertions; seven distinct approved S-01 source hashes/URLs and image captures; DOM overlay absence | square-corner realistic Cover hero treatment is a named invariant | S-01/S-02 scoped passed by UNIT-02-C1 with seven distinct artworks; S-10/S-11/S-12 scoped by UNIT-03; S-02-unavailable/S-13/S-18 scoped by UNIT-04; S-04/S-06 order-cover instances rendered the unchanged square 2:3 assets in UNIT-05 receipts; later routes remain with their owning units |
...
| VIS-BRAND-LOGO | `AVB-UKIEBOOK-AURORA-7B-V3` / `e50c9f82c241195d7f5d8876d9dcdcd7fd45b71cdaf6d2eedfe2e327a7182724` | all rendered logo instances, default/focus/high-density, 390/768/1280/1440 | P2 / blocking | active transparent SVG only; no backing plate; locked brand geometry preserved | source SHA-256 `db838dd4ad696f63cccb6aa86ab98e53dc5c6e13c1778ac340f62c5e4514617f`, alpha/background inspection + captures against V3 | explicit transparent-logo correction; raster-backed container is not claimed as path-vector | S-01/S-02 passed by UNIT-02-C1; S-10/S-11/S-12 scoped by UNIT-03; S-02-unavailable/S-13/S-18 scoped by UNIT-04; S-04/S-05/S-06 rendered the unchanged shared official asset in UNIT-05 extension receipts; other visible routes require a current V3 receipt when touched |
...
- UNIT-05 separately passes only S-04/S-05/S-06 Cart/payment/`PaidSale`/E-01 through the UNIT-06 handoff. It does not claim Library entitlement/files, reviews, Refund compensation, rewards/ledger, production transactional-email configuration, credentialed mono/registered webhook, Apple Pay/Google Pay presentation, production tariffs or complete WCAG/cross-browser/device coverage.
- UNIT-06 separately passes Library entitlement/files, verified-Buyer reviews and Refund compensation producer/UI scope. It does not claim UNIT-07 rewards-ledger application, production S3, production-provider activation or complete WCAG/cross-browser/device coverage.
- Signed local mono protocol simulation does not confirm credentialed provider configuration, Apple Pay/Google Pay presentation or production tariffs; those close only with the activation smoke/current provider evidence and OQ-2.
...
Blockers: product implementation units UNIT-07…UNIT-10 and the unimplemented portions of S-14…S-16/S-19/S-21 plus complete deployment-context release-journey evidence are not complete; later-owned S-13 Discount/Update states remain unimplemented. Credentialed Google/Facebook and mono/wallet smoke block their production-provider activation; production transactional email, legal/tax review (OQ-1), mono production terms (OQ-2), and production font license/provenance remain open. UNIT-00 through UNIT-06 are current only within their recorded scopes. MOBI, moderation/publication, Cart/payment/PaidSale/E-01 and Library/review/Refund proofs are closed; next executable unit is UNIT-07 for rewards ledger, payouts and Founder Author.
