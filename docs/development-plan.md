...
- `docs/design-brief.md` — Approved Baseline `AVB-UKIEBOOK-AURORA-7B-V1`; target bundle hash `758b1a6a78db1226bf82244698d74b44c90b9b2779b072c1bada0d00abd4f5f7`; tree hash `950a44c329ebd4b79d088910920d05abb81a683c4e1b53f6eebda820905626f0`; permitted variance.
...
- `forge/design/README.md` and immutable target artifacts — visual reference only, no runtime truth.
...
- **Acceptance Checks:** all six npm commands exist; build/lint/typecheck/unit smoke run locally; migrations can apply/rollback on an empty database; worker claims one test job idempotently; no secret enters client bundle; token snapshot equals design-brief values.
- **Verification:** DoD `build`, `typecheck_lint`, `tests`; architecture boundary review; `npm run test:visual` can capture a fixture route.
...
- **Approved Baseline ID:** `AVB-UKIEBOOK-AURORA-7B-V1`.
- **Immutable Visual Target Hash:** `758b1a6a78db1226bf82244698d74b44c90b9b2779b072c1bada0d00abd4f5f7`.
...
- **Visual Fidelity Verification:** `approved_visual_baseline_fidelity` via `VIS-AURORA-PUBLIC`, `VIS-AURORA-AUTHOR`, affected RES/A11Y.
- **Prototype Reuse:** none.
...
- **Source References:** FR-CAT-1..4, FR-AUTH-2, US-001/004; S-01/S-02; wireframes S-01/S-02; Approved Baseline and official `UkieBook-logo.jpg` override; architecture AD-8; QA ACC-08, ST-01, WF-02/04, UX-03/04/07, VIS-S01-*, VIS-BRAND-LOGO.
...
- **Verification:** ACC-08; ST-01; WF-02/04; UX-03/04/07; VIS-S01-1280-DEFAULT/HOVER/RESPONSIVE; VIS-AURORA-PUBLIC; VIS-BRAND-LOGO; RES-01/02; A11Y-01/02/04/06/08; INT-04.
- **Delivery Layer:** full-stack.
- **Approved Baseline ID:** `AVB-UKIEBOOK-AURORA-7B-V1`.
- **Immutable Visual Target Hash:** `758b1a6a78db1226bf82244698d74b44c90b9b2779b072c1bada0d00abd4f5f7`.
...
- **Visual Fidelity Verification:** `approved_visual_baseline_fidelity` with all `VIS-S01-*` and `VIS-AURORA-PUBLIC` checks.
- **Prototype Reuse:** none.
...
- **Visual Fidelity Verification:** `approved_visual_baseline_fidelity` via `VIS-AURORA-AUTHOR`, RES-04, A11Y forms/dialogs.
- **Prototype Reuse:** none.
...
- **Visual Fidelity Verification:** `approved_visual_baseline_fidelity` via `VIS-AURORA-AUTHOR/MANAGER/PUBLIC` and UX-05/06.
- **Prototype Reuse:** none.
...
- **Operator Visual Overrides:** final 7b system.
- **Visual Fidelity Verification:** `approved_visual_baseline_fidelity` via `VIS-AURORA-PUBLIC`, RES-03, A11Y/INT.
- **Prototype Reuse:** none.
...
- **Operator Visual Overrides:** final 7b system.
- **Visual Fidelity Verification:** `approved_visual_baseline_fidelity` via `VIS-AURORA-PUBLIC/MANAGER`, RES-03/05, A11Y-07.
- **Prototype Reuse:** none.
...
- **Source References:** FR-REW-1..6, FR-PYT-1..4, FR-FND-1..4, FR-UPD-2; US-009/011/012; architecture AD-2/7; QA ACC-12..14, ST-06/08/10.
...
- **Visual Fidelity Verification:** `approved_visual_baseline_fidelity` via `VIS-AURORA-AUTHOR/MANAGER`, UX-02/05, RES-04/05.
- **Prototype Reuse:** none.
...
- **Operator Visual Overrides:** final 7b system.
- **Visual Fidelity Verification:** `approved_visual_baseline_fidelity` for affected public/Author/Manager checks.
- **Prototype Reuse:** none.
...
- **Work Items:** full route/state capture matrix; exact S-01 diffs; official-logo source/derivative hash and shape comparison; measured contrast corrections within variance; semantic/control/focus audit; target-size/zoom/reflow fixes; mobile master-detail/table/form layouts; reduced-motion verification; cross-browser critical journeys; canonical copy sweep; regression snapshot baselines keyed by Baseline ID/hash.
...
- **Verification:** `npm run test:visual`, `npm run test:e2e`, VIS-S01-*, VIS-AURORA-*, VIS-BRAND-LOGO, RES-01..06, A11Y-01..08, BD-01/02, `approved_visual_baseline_fidelity` for every visible unit.
...
- Official logo: verify every rendered instance against `UkieBook-logo.jpg`; optimization/background removal may not redraw or distort the mark, and S-01 header placement may not alter locked geometry.
