...
- **Verification tooling:** Vitest `4.1.10` and Playwright `1.61.1`; stable commands `npm run build`, `npm run lint`, `npm run typecheck`, `npm test`, `npm run test:e2e`, `npm run test:visual` are executable. Historical UNIT-00…UNIT-03 receipts retain their revision-bound contracts. `npm run verify:unit04` passed on committed revision `4552048aeb2ba6da16b47ac289058b14d5641869` over dedicated loopback PostgreSQL database `ukiebook_unit04`, reversible schema revision `0005_moderation_publication`, 16 moderation/publication proof vectors, 109 passed/3 skipped tests, 4/4 E2E, 50 S-13/S-18/S-02 visual receipts and 8 accessibility checks; canonical evidence is `forge/runs/UNIT-04/20260722T190601Z-4552048aeb2b/run.json`. Any affected architecture change reruns that contract; the receipt does not expand into later commerce/rewards/library scopes.
