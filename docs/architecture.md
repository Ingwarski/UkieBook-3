...
| `commerce` | Persistent guest/Buyer Кошик, immutable Order/OrderItem price snapshots, платіжні сесії mono, signed webhook observations, reconciliation issues and append-only `PaidSale`. Refund request/decision/compensation extends this boundary in UNIT-06 and is not yet implemented. | S-04..S-06; future S-09/S-20 |
...
Правила меж: `publishing` емітить immutable `BookSubmitted`; `moderation` вибирає активну версію й атомарно записує publication pointer + Catalog projection, а public `catalog` ніколи не читає приватний draft/object graph напряму. `commerce` емітить versioned append-only `PaidSale`, але не створює entitlement; UNIT-06 `library` є його наступним споживачем. `rewards` слухає події `commerce` (оплата/майбутнє повернення) і `publishing` (платне оновлення), але не пише в них; `reviews` перевіряє покупку через інтерфейс `library`; ПД покупців не перетинають межу `rewards`-звітів автора (FR-REW-6).

## Runtime And Automation Model

- Веб-процес: синхронні запити UI + API.
- Воркер асинхронних задач: конвертація UNIT-03; UNIT-04 moderation relay/handler; UNIT-05 commerce reconciliation and purchase-notification delivery. Перед poll воркер ідемпотентно перетворює unpublished `BookSubmitted` outbox events на `moderation_cases` + durable `moderation.screen.v1`; clear screening активує публікацію, flagged або provider failure safe-fail-ить у `manual_review_pending`. Commerce jobs звіряють незавершені provider sessions beyond webhook arrival/outage and always enqueue a durable successor until authoritative terminal resolution. Notification jobs consume `PurchaseNotificationRequested` independently, so provider failure never rolls back `PaidSale`.
- Вебхук-приймач mono: bounded exact raw body + ECDSA signature verification; invoice/reference/amount/currency binding; idempotent/out-of-order-safe observation; successful terminal transition atomically writes paid Order + append-only `PaidSale` + outbox. Failure/cancel preserves Cart and creates no `PaidSale`; Refund compensation remains UNIT-06.
...
- **Verification tooling:** Vitest `4.1.10` and Playwright `1.61.1`; stable commands `npm run build`, `npm run lint`, `npm run typecheck`, `npm test`, `npm run test:e2e`, `npm run test:visual` are executable. Historical UNIT-00…UNIT-04 receipts retain their revision-bound contracts. `npm run verify:unit05` passed on committed revision `92e820ffc4e5c69752543457ad7fecf12e960c15` over dedicated embedded loopback PostgreSQL database `ukiebook_unit05`, reversible schema revision `0006_commerce_checkout`, 28 commerce proof vectors, 129 passed/3 skipped tests, 3/3 Buyer E2E, 30 S-04/S-05/S-06 visual receipts and 8 accessibility checks; canonical evidence is `forge/runs/UNIT-05/20260727T215803Z-92e820ffc4e5/run.json`. Any affected architecture change reruns that contract; the receipt does not expand into Library/reviews/Refund/rewards or credentialed production-provider activation.
