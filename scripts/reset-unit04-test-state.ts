...
import {
  ...
  UNIT04_DATABASE_NAME,
} from "./unit04-database-guard";

if (process.env.APP_ENV !== "test") {
  throw new Error("UNIT-04 test-state reset requires APP_ENV=test");
}
if (process.env.UNIT04_ALLOW_TEST_RESET !== "1") {
  throw new Error(
    "Set UNIT04_ALLOW_TEST_RESET=1 to acknowledge the destructive reset",
  );
}
...
const roots = [
  ...
  path.resolve(
    process.env.UNIT04_PUBLIC_ASSET_ROOT ??
      process.env.PUBLIC_CATALOG_ASSET_ROOT ??
      ".data/unit04-e2e-public",
  ),
];
...
for (const root of roots) {
  ...
  if (
    ...
    !relative.startsWith("unit04-") ||
    relative === "unit04-"
  ) {
    throw new Error(
      "UNIT-04 reset requires direct .data/unit04-* artifact directories",
    );
  }
}
